...
from .spectrogram_label_widgets import SpectrogramWidget
from .psd_widget import PSDWidget


class PreviewWindow(QWidget):
    """Main window for data preview."""
    ...
    def __init__(self, electrode_positions=None, current_data=None, 
        ...
        self.epoch_length = 5.0  # Default to 5 seconds
        self.sampling_rate = self.detected_sampling_rate or 200.0
    ...
    def init_ui(self):
        ...
        # self.showFullScreen() # User might prefer windowed mode for preview
        ...
        # Right Upper: Mosaic Plotter (0.7 height)
        ...
        # Right Lower: Spectrogram (0.3 height)
        ...
        # Right Lower: PSD Graph (0.2 height)
        self.psd_widget = PSDWidget(self.theme_colors)
        if not self.df.empty:
            first_channel_data = self.df.iloc[:, 0].values
            self.psd_widget.set_data(first_channel_data, self.sampling_rate, self.epoch_length)
        right_layout.addWidget(self.psd_widget, 20) # 20% stretch
    ...
    def create_control_panel(self):
        ...
        self.summary_label_2 = QLabel(summary_text_2)
        self.summary_label_2.setWordWrap(True)
        summary_layout.addWidget(self.summary_label_2)
        
        layout.addWidget(summary_group)
        
        # Epoch Length Control
        epoch_group = QGroupBox("Epoch Configuration")
        epoch_layout = QFormLayout(epoch_group)
        
        self.epoch_spin = QDoubleSpinBox()
        self.epoch_spin.setRange(0.01, 10000.0)
        self.epoch_spin.setValue(self.epoch_length)
        ...
        topo_info = QLabel("Disable to speed up preview")
    ...
    def update_epoch_displays(self, update_label_view=True):
        ...
        # Update PSD widget
        self.psd_widget.set_epoch(self.current_epoch)
    ...
    def on_epoch_double_clicked(self, epoch_idx):
        """Handle double-click on epoch in mosaic plotter."""
        self.current_epoch = epoch_idx
        self.update_epoch_displays()
    ...
    def on_epoch_length_changed(self, value):
        ...
        self.mosaic_plotter.set_epoch_length(self.epoch_length)
        self.spectrogram_widget.set_epoch_length(self.epoch_length)
        self.psd_widget.set_epoch_length(self.epoch_length)
        
        # Update data summary label
        if hasattr(self, 'summary_label_2'):
            n_epochs = self.get_n_epochs()
            summary_text_2 = (f"Epoch Length: {self.epoch_length:.1f} sec | "
                           f"Total Epochs: {n_epochs}")
            self.summary_label_2.setText(summary_text_2)
            
        # Re-initialize or update displays
        self.update_epoch_displays()
        
        # Update spectrogram view if needed (it depends on total epochs)
        if self.spectrogram_epochs_to_show is None or self.spectrogram_epochs_to_show > self.get_n_epochs():
            self.spectrogram_epochs_to_show = self.get_n_epochs()
            self.spectrogram_widget.set_epochs_to_show(self.spectrogram_epochs_to_show)
