...
class SpectrogramWidget(QWidget):
    ...
    def precompute_spectrogram(self):
        ...
        for epoch_idx in range(n_epochs):
            ...
            # Adapt nperseg for very short epochs
            nperseg_val = min(len(epoch_data), int(self.sampling_rate))
            if nperseg_val < 256 and len(epoch_data) >= 32:
                 # For short segments that are still usable, use full length
    ...
    def update_plot(self):
        ...
        # Adjust start if end is clipped to ensure full window width if possible
    ...
    def set_epoch_length(self, epoch_length):
        ...
        if self.epoch_length != epoch_length:
            ...
            # Must re-compute spectrogram since bin sizes changed
    ...
    def on_click(self, event):
        ...
        # The x-axis is already in Epoch units, so no need to divide by epoch_length
        clicked_epoch = int(clicked_time)
    ...
    def toggle_selection_mode(self):
        """Toggle the epoch selection mode."""
